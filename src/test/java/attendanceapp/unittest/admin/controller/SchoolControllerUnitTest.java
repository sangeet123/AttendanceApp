package attendanceapp.unittest.admin.controller;
...
import attendanceapp.unittest.admin.testdata.SchoolControllerTestData;
...
public class SchoolControllerUnitTest extends UnitTestConfigurer {
	...
	public void getSchool_ShouldReturnAllRegisteredSchools() throws Exception {
		...
		List<SchoolResponseObject> testSchools = SchoolControllerTestData
				.getTestSchoolList();

		when(schoolServiceMock.getSchoolList()).thenReturn(testSchools);

		ResultActions resultAction = getMockMvc()
				.perform(get(SchoolControllerUnitTestConstants.GETALLSCHOOLS))
				.andExpect(status().isOk())
				.andExpect(
						content()
								.contentType(
										AttendanceAppUnitTestUtil.APPLICATION_JSON_UTF8))
				.andExpect(jsonPath("$", hasSize(testSchools.size())));

		for (int i = 0; i < testSchools.size(); ++i) {
			String jasonPath = String.format(
					AttendanceAppUnitTestUtil.JASONLIST, i);
			SchoolResponseObject schoolResponseObject = testSchools.get(i);
			resultAction.andExpect(jsonPath(
					jasonPath.concat(SchoolControllerUnitTestConstants.ID),
					is((int) schoolResponseObject.getId())));
			resultAction.andExpect(jsonPath(
					jasonPath.concat(SchoolControllerUnitTestConstants.NAME),
					is(schoolResponseObject.getName())));
			resultAction.andExpect(jsonPath(
					jasonPath.concat(SchoolControllerUnitTestConstants.EMAIL),
					is(schoolResponseObject.getEmail())));
			resultAction.andExpect(jsonPath(jasonPath
					.concat(SchoolControllerUnitTestConstants.TELEPHONE),
	...
	@Test
	public void getSchoolById_ShouldReturnSchool_for_valid_id_and_school_that_exist()
			throws Exception {

		long id = 5;
		List<SchoolResponseObject> schoolsResponseObject = SchoolControllerTestData
				.getSchool(id);
		SchoolResponseObject returnedSchoolResponseObject = schoolsResponseObject
				.get(0);
		when(schoolServiceMock.getSchool(id)).thenReturn(
				returnedSchoolResponseObject);

		getMockMvc()
				.perform(
						get(SchoolControllerUnitTestConstants.GETSCHOOLWITHID,
								id))
				.andExpect(status().isOk())
				.andExpect(
						content()
								.contentType(
										AttendanceAppUnitTestUtil.APPLICATION_JSON_UTF8))
				.andExpect(
						jsonPath(AttendanceAppUnitTestUtil.JASONUNIT
								.concat(SchoolControllerUnitTestConstants.ID),
								is((int) returnedSchoolResponseObject.getId())))
				.andExpect(
						jsonPath(
								AttendanceAppUnitTestUtil.JASONUNIT
										.concat(SchoolControllerUnitTestConstants.NAME),
								is(returnedSchoolResponseObject.getName())))
				.andExpect(
						jsonPath(
								AttendanceAppUnitTestUtil.JASONUNIT
										.concat(SchoolControllerUnitTestConstants.EMAIL),
	...
	public void getSchoolById_ShouldReturnHttpStatusCode404_for_valid_id_but_school_that_does_not_exist()
		...
		when(schoolServiceMock.getSchool(validIdThatDoesNotExist)).thenThrow(
				new SchoolNotFoundException(
						SchoolRestControllerConstants.SCHOOL_DOES_NOT_EXIST));
		getMockMvc().perform(
				get(SchoolControllerUnitTestConstants.GETSCHOOLWITHID,
						validIdThatDoesNotExist)).andExpect(
				status().isNotFound());
	...
	@Test
	public void getSchoolById_ShouldReturnHttpStatusCode404_for_id_that_is_invalid()
			throws Exception {
		long invalidSchoolId = -1L;
		when(schoolServiceMock.getSchool(invalidSchoolId)).thenThrow(
				new SchoolNotFoundException(
						SchoolRestControllerConstants.SCHOOL_DOES_NOT_EXIST));
		getMockMvc().perform(
				get(SchoolControllerUnitTestConstants.GETSCHOOLWITHID,
						invalidSchoolId)).andExpect(status().isNotFound());
	...
	public void deleteSchoolById_ShouldReturnHttpStatusCode404_for_id_that_is_valid_but_does_not_exist()
		...
		getMockMvc().perform(
				delete(SchoolControllerUnitTestConstants.DELETESCHOOL,
						validSchoolIdThatDoesNotExist)).andExpect(
				status().isNotFound());
		verify(schoolServiceMock, times(1)).delete(
				validSchoolIdThatDoesNotExist);
		verifyNoMoreInteractions(schoolServiceMock);
	}

	@Test
	public void deleteSchoolById_ShouldReturnHttpStatusCode404_for_id_that_is_invalid()
			throws Exception {
		long invalidSchoolId = -1L;
		doThrow(
				new SchoolNotFoundException(
	...
	@Test
	public void createSchool_ShouldReturnValidationError_For_Empty_School_In_English_()
			throws Exception {
		SchoolRequestObject schoolRequestObject = new SchoolRequestObject();

		getMockMvc()
				.perform(
						post(SchoolControllerUnitTestConstants.CREATESCHOOL)
								.contentType(
										AttendanceAppUnitTestUtil.APPLICATION_JSON_UTF8)
								.content(
										AttendanceAppUnitTestUtil
												.convertObjectToJsonBytes(schoolRequestObject)))
				...
				.andExpect(
						jsonPath(
								"$.fieldErrors[*].field",
								containsInAnyOrder("telephone", "name",
		...
		getMockMvc()
				...
				.andExpect(
						jsonPath(
								"$.fieldErrors[*].message",
								containsInAnyOrder(
										"School email is not a valid email. Please enter the valid email address.",
