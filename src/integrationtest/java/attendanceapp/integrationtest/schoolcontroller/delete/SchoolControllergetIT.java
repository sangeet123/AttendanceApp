...
public class SchoolControllergetIT extends TestConfigurerIT {
	...
	public void setUp() {
		...
		if (!isSettedUp) {
			try {
				AttendanceAppUtilIT
						.mysqlScriptRunner(insertSchoolQuerySQLScriptFilePath);
	...
	public void get_school_with_valid_id_1_that_exist() throws Exception {
		...
		getMockMvc()
				.perform(
						get(SchoolControllerConstantsIT.GETSCHOOLWITHID,
								validSchoolId))
				.andExpect(status().isOk())
				.andExpect(
						content().contentType(
								AttendanceAppUtilIT.APPLICATION_JSON_UTF8))
				.andExpect(
						content()
								.string("{\"id\":1,\"name\":\"test1\",\"telephone\":\"123456789\",\"email\":\"test1@email.com\"}"));
	}
	...
	public void get_school_with_valid_id_10_that_exist() throws Exception {
		...
		getMockMvc()
				...
				.andExpect(
						content()
								.string("{\"id\":10,\"name\":\"test10\",\"telephone\":\"234567891\",\"email\":\"test10@email.com\"}"));
	}

	@Test
	public void get_school_with_valid_id_15_that_does_not_exist()
			throws Exception {
		final long validIdThatDoesNotExist = 15L;
		getMockMvc()
				.perform(
						get(SchoolControllerConstantsIT.GETSCHOOLWITHID,
								validIdThatDoesNotExist))
				...
				.andExpect(
						content()
								.string("{\"statusCode\":2,\"message\":[\"School does not exist.\"]}"));
	}
	...
	public void get_school_with_invalid_id_one() throws Exception {
		...
		getMockMvc()
				...
				.andExpect(
						content()
								.string("{\"statusCode\":2,\"message\":[\"Resource does not exist.\"]}"));
	}
}
